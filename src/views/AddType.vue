<template>
  <section class="field">
          ...
          <div class="file is-centered is-boxed is-success has-name">
            <label class="file-label">
              <input
                ...
                type="file"
                name="resume"
                v-on:change="file_upload($event, 'type')"
                required
              />
              <span class="file-cta">
    ...
    <section class="container">
      <div class="columns is-flex-wrap-wrap" v-if="category_data[0]">
        <div
          class="column card is-2 m-3"
          v-for="cat in category_data[3].cartype"
          :key="cat.id"
        >
          <div class="column">
            <div
...
const thumbnail = ref([]);
const edit_id = ref();
const model_type = ref("type");
const file_upload = (e, key) => {
  key === "type" ? (thumbnail.value = e.target.files[0]) : "";
};
...
const add_category = async () => {
  ...
  model_type.value === "type"
    ? await axios
        ...
        .catch((e) => console.log(e))
    : await axios
        .patch(`api/v1/admin/type/` + edit_id.value, fm)
        .then((res) => {
          store.dispatch("fetchAdmin");
          add_category_.value = "";
          add_category_slug.value = "";
